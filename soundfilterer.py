...
class SoundFilterer:
    ...
    def ffmpeg_convert(self, input_path, output_path):
        subprocess.call([str(pathlib.Path().absolute())+'/ffmpeg.exe',
                        '-i', input_path, output_path, '-y'], shell=True)
    ...
    async def prepare_audio_file_for_filtering(self):
        ...
        self.sound_duration, self.sampling_rate, self.sound_channels = self.sound_info[
            1], self.sound_info[2], self.sound_info[3]
        self.sound_file_format = ['WAVE', 'AIFF', 'AU', 'RAW',
                                  'SD2', 'FLAC', 'CAF', 'OGG'].index(self.sound_info[4])
        self.samptype = ['16 bit int', '24 bit int', '32 bit int', '32 bit float',
                         '64 bits float', 'U-Law encoded', 'A-Law encoded'].index(self.sound_info[5])
        ...
        self.audio_server.recordOptions(dur=self.sound_duration, filename="sound.wav",
                                        fileformat=self.sound_file_format, sampletype=self.samptype)
    ...
    def clean_all_temp_files(self):
        possible_temp_files = ["sound.mp3",
                               "sound.wav", "temp,mp3", "temp.wav"]
    ...
    async def add_yoi(self):
        ...
        sound_fmed = IRFM(self.sound_file, carrier=5000,
                          ratio=lfo, index=3, order=256).out()
    ...
    async def max_distortion(self):
        ...
        sound_mixed = Interp(
            self.sound_file, sound_distorted, interp=0.5, mul=0.5).out()
        await self.render_filtered_audio_file()
